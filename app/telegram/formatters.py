...
def format_balance(b: dict) -> str:
    ...
    return (
        ...
        f"Ставок: <b>{b['bets']}</b> (✅ {b['won']} / ❌ {b['lost']})  ·  стейк {b['stake_pct']:.1f}% банка\n\n"
        "<i>Ставим на наш прогноз (фаворита) по рыночному кэфу — кэф нужен только "
        "для расчёта выплаты. Размер ставки — % от текущего банка.</i>"
    )
